...
public abstract class BaseField implements Field{
    ...
    public String getInfo() {
        ...
        return String.format("%s (%s):", name, this.getClass().getSimpleName()) + System.lineSeparator() +
                String.format("Player: %s", (players.size() == 0 ? "none" : players.stream().map(Player::getName).collect(Collectors.joining(" ")))) + System.lineSeparator() +
                String.format("Supplement: %d", supplements.size()) + System.lineSeparator() +
                String.format("Energy: %d", this.sumEnergy());
    }
